...
const aiTokenMigratedController = {
  // ✅ API: Ambil data migrated dari API eksternal pakai token dari DB
  getMigratedData: async (req, res) => {
    try {
      ...
      // Body request ke API eksternal untuk migrated
      ...
      console.error("❌ Error fetching Migrated data:", error.message);
      return res.status(500).json({
        success: false,
        message: "Failed to fetch migrated data from external API",
        error: error.message
      });
    }
  }
...
module.exports = aiTokenMigratedController;
