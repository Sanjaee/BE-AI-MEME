...
const aiTokenBnbMigratedController = {
  // ✅ API: Ambil data migrated BNB dari API eksternal pakai token dari DB
  getMigratedData: async (req, res) => {
    try {
      ...
      // Body request ke API eksternal untuk migrated BNB
      ...
      console.error("❌ Error fetching BNB Migrated data:", error.message);
      return res.status(500).json({
        success: false,
        message: "Failed to fetch BNB migrated data from external API",
        error: error.message
      });
    }
  }
...
module.exports = aiTokenBnbMigratedController;
