...
const aiTokenBnbNewpairs = {
  // ✅ API: Ambil data newPairs BNB dari API eksternal pakai token dari DB
  getNewPairsData: async (req, res) => {
    try {
      ...
      // Body request ke API eksternal untuk newPairs BNB
      ...
      console.error("❌ Error fetching BNB NewPairs data:", error.message);
      return res.status(500).json({
        success: false,
        message: "Failed to fetch BNB newPairs data from external API",
        error: error.message
      });
    }
  }
...
module.exports = aiTokenBnbNewpairs;
